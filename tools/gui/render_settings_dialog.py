...
class RenderSettingsDialog(QDialog):
    def __init__(
        ...
        *,
        ...
        self.width_spin.setValue(schema.render.width)
        self.height_spin = QSpinBox()
        self.height_spin.setRange(64, 8192)
        self.height_spin.setValue(schema.render.height)
        self.spp_spin = QSpinBox()
        self.spp_spin.setRange(1, 4096)
        self.spp_spin.setValue(schema.render.spp)
        self.max_depth_spin = QSpinBox()
        self.max_depth_spin.setRange(1, 64)
        self.max_depth_spin.setValue(schema.render.max_depth)
        self.adaptive_check = QCheckBox("Adaptive")
        self.adaptive_check.setChecked(schema.render.adaptive)
        self.adaptive_threshold = QLineEdit(_float_text(schema.render.adaptive_threshold))
        self.denoise_check = QCheckBox("Denoise")
        self.denoise_check.setChecked(schema.render.denoise)
        self.lighting_preset_combo = QComboBox()
        self.lighting_preset_combo.addItems(schema.choices.lighting_preset)
        lighting_idx = self.lighting_preset_combo.findText(schema.defaults.lighting_preset)
        if lighting_idx >= 0:
            self.lighting_preset_combo.setCurrentIndex(lighting_idx)
        if not schema.capabilities.supports_lighting_preset:
        ...
        self.ppm_denoised_only.setChecked(schema.render.ppm_denoised_only)
        self.atmosphere_check = QCheckBox("Enable fog / atmosphere")
        self.atmosphere_check.setChecked(schema.render.atmosphere_enabled)
        self.atmosphere_density = QLineEdit(_float_text(schema.render.atmosphere_density))
        self.atmosphere_color = QLineEdit(schema.render.atmosphere_color)

        self.keep_frames = QCheckBox("Keep individual frame files")
        self.keep_frames.setChecked(schema.defaults.video_keep_frames)
        ...
        self.scatter_spacing_edit = QLineEdit(_float_text(schema.scene_overrides.scatter_spacing))
        ...
        self.scatter_max_radius_edit = QLineEdit(_float_text(schema.scene_overrides.scatter_max_radius))
        ...
        self.scatter_growth_scale_edit = QLineEdit(_float_text(schema.scene_overrides.scatter_growth_scale))
        ...
        self.big_scatter_spacing_edit = QLineEdit(_float_text(schema.scene_overrides.big_scatter_spacing))
        ...
        self.big_scatter_max_radius_edit = QLineEdit(_float_text(schema.scene_overrides.big_scatter_max_radius))
        ...
        self.row_scatter_xmax_edit = QLineEdit(_float_text(schema.scene_overrides.row_scatter_xmax))
        ...
        self.row_scatter_z_front_edit = QLineEdit(_float_text(schema.scene_overrides.row_scatter_z_front))
        ...
        self.row_scatter_z_back_edit = QLineEdit(_float_text(schema.scene_overrides.row_scatter_z_back))
        ...
        self.row_scatter_replacement_rate_edit = QLineEdit(_float_text(schema.scene_overrides.row_scatter_replacement_rate))
